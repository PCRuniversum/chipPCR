...
fig1_scap <- "Simulation of a qPCR experiment using \\textsl{AmpSim} function."
...
an right neighbors above and the below the used defined threshold level 
are use."
fig2_scap <- "Working principle of \\textsl{th}.\\textsl{cyc}."

fig3_cap <- "Application of \\textsl{th}.\\textsl{cyc} for the analysis of ccPCR data.
Data from a ccPCR were analyzed using the th.cyc function 
...
fig3_scap <- "Application of \\textsl{th}.\\textsl{cyc} for the analysis of ccPCR data."

fig4_cap <- "Application of the \\textsl{CPP} and \\textsl{th}.\\textsl{cyc} functions.
...
Cqs were calculated with th.cyc function. The Cq for the raw data was 
...
fig4_scap <- "Application of the \\textsl{CPP} and \\textsl{th}.\\textsl{cyc} functions."
...
linear regression ($trans = TRUE$). \\emph{(B)} baselined raw data, 
...
\\emph{(A)} AmpSim was used to synthesize a qPCR experiment 
of six dilutions (three replicates per dilution) standard samples. The Cqs 
...
The regression curve (\\textendash) was plotted as the decadic logarithm 
of input concentration versus the Cq. The 95\\% confidence interval is 
shown be the light-blue solid lines."
fig6_scap <- "Amplification standard curve simulation and regression analysis."
...
fig9_cap <- "Cycle of quantification by the second derivative maximum method.
Raw data (\\textbullet) were generated using the AmpSim 
simulation function (see example main text). The inflection point is the 
point where the slope is maximum and the curvature is zero. The first 
derivative of the amplification curve has a first derivative maximum 
($FDM$) at the inflection point.The second derivative maximum method 
($SDM$) needs to differentiate a curve to the second order prior to 
quantification. The second derivative exhibits a zero-crossing at the 
$FDM$. The function $y = f(x)$ is numerically derived by five-point 
stencil. This method do not require any assumptions regarding the function 
f. The function inder calculates the approximate $SDM$. The $SDM$ might in 
addition be useful for isothermal amplification processes. The $SDM$ is 
calculated from a derived cubic spline. Similarly the first approximate 
approximate derivative maximum ($FDM$), second derivative minimum ($SDm$), 
and approximate second derivative center ($SDC$, geometric mean of $SDM$ 
and $SDm$) are available. $FDM$, $SDm$ and $SDC$ values can be used to 
further characterize the amplification process."
fig9_scap <- "Cycle of quantification by the second derivative maximum method."
...
fig11_cap <-  "Amplification curve profiles from the Bio-Rad iQ5 thermo cycler 
...
fig11_scap <- "Amplification curve profiles from the Bio-Rad iQ5 thermo cycler 
for the human gene \\textit{HPRT1}"
...
The VideoScan Platform was used to 
monitor the amplification. The HDA was performed at 65 degree Celsius. 
Three concentrations of input DNA (D1, D2, D3) were used. The amplifcation 
curves were smoothed by a moving average (windowsize 3) and baselined by a 
a robust linear regression by computing MM-type regression estimator. The 
th.cyc function was used to determine the time required to reach the 
...
Celsius (annealing phase; A, B) and 69 degree Celsius (elongation phase, C, 
D). First cycles 1 to 10 of 96 qPCR replicate amplification curves were 
analyzed. Next the cycles 1 to 40 of the same amplification curve data were 
analyzed. The Breusch-Pagan confirmed the heteroskedasticity in the 
...
the \textsl{bg.max()} function can be added to the plots to provide even 
more knowledge about the properties of analyzed experiments. Before the 
start of any analysis, a user must must choose data set and proper tools. 
...
distribution. This functions enables the analysis of specific parts of the 
...
standard deviation. \textsl{MFIaggr} has parameter $llul$ to define the 
...
into the the characteristics of a system. In the following example we 
...
VIMCFX96\_60 data set were obtained during the annealing phase at 60 degree 
...
hsk.test <- function(x, y, llul = c(1,15), main = "") {
    ...
    format(summary(res)[5], digits = 2)), bty = "n")
}
...
<<plotCurves,fig.show='hold',fig.cap=fig14_cap,fig.scap=fig14_scap>>=
y <- VIMCFX96_60[, 2L:9]
...
estimation based on the approx function. This approach is useful but may 
be 
...
Eilers 2003]. The function fixNA imputes missing values in a single column 
of data. The imputation is based on a linear approximation by default. 
...
\textsl{normalizer} is a function to normalize any data set. It is possible 
...
amplification curve.
\item minmax does a normalization between 0 and 1 (see \cite{roediger_RJ_2013} for explanation).
...
smoother (see the 
\textsl{smoother} function for details).

Provided that the smoother is 
...
\textsl{th.cyc} is a function to calculate the number of cycles at which 
the 
...
\subsubsection{Application of the th.cyc and CPP function for Helicase Dependent Amplification}
\label{sec:HDA}
...
<<HDA,fig.show='hold',fig.cap=fig13_cap,fig.scap=fig13_scap>>=
par(mfrow = c(2,1), bty = "n")
plot(NA, NA, xlim = c(0,5000), ylim = c(0,1), xlab = "Time [sec]", 
     ylab = "Fluorescence", main = "HDA amplification - Raw data")
mtext("A", cex = 2, side = 3, adj = 0)
...
plot(NA, NA, xlim = c(0,2000), ylim = c(0,0.4), xlab = "Time [sec]", 
     ylab = "Fluorescence", main = "HDA amplification - Pre-processed data")
mtext("B", cex = 2, side = 3, adj = 0)
legend("topleft", c("D1, 1x", "D2, 1:10", "D3, 1:100"), col = c(2,4,6), 
	  pch = rep(20,3))
...
used to rate a curve for a certain characteristic. \textsl{humanrater} draws
...
\textsl{lm.coefs} is convenient wrapper around functions performing 
normal (least squares) and robust linear regression. If the robust linear 
regression is not possible, \textsl{lm.coefs} will perform linear regression 
...
datdf <- structure(list(dat.names = c("capillaryPCR", "C60.amp", 
"C60.melt", 
...
"CD75", "C81", "C85"), descrs = c("  The capillary convective PCR (ccPCR) 
is a modified device of the ccPCR system proposed by .", 
"  qPCR Experiment for the Amplification of MLC-2v and Vimentin (as 
decadic dilutions) Using the Roche Light Cycler 1.5.", 
"  Melt Curves MLC-2v and Vimentin for the qPCR experiment C60.amp using 
the Roche Light Cycler 1.5", 
"  A Quantitive PCR (qPCR) with the DNA binding dye (EvaGreen) 
was performed in the Bio-Rad iQ5 thermo cycler. The cycle-dependent 
increase of the fluorescence was quantified at the elongation step (59.5 
deg Celsius).", 
"  A Quantitive PCR (qPCR) with the DNA binding dye (EvaGreen)
was performed in the Bio-Rad iQ5 thermo cycler. The 
cycle-dependent 
increase of the fluorescence was quantified at the elongation step (68.5 
deg Celsius).", 
"  Quantitive PCR (qPCR) with a hydrolysis probe (Cy5/BHQ2) and DNA 
binding dye (EvaGreen)  performed in the Roche Light 
Cycler 1.5 thermo cycler.", 
"  Human vimentin amplifcation curve data (measured during annealing phase 
at 60 deg Celsius) for 96 replicate samples in a Bio-Rad CFX96 thermo 
cycler.", 
"  Human vimentin amplifcation curve data (measured during elongatio phase 
at 69 deg Celsius) for 96 replicate samples in a Bio-Rad CFX96 thermo 
cycler.", 
"  Human vimentin melting curve data for 96 replicate samples in a Bio-Rad 
CFX96 thermo cycler.", 
"  Human vimentin amplifcation curve data (measured during annealing phase 
at 59.5 deg Celsius) for 96 replicate samples in a Bio-Rad iQ5 thermo 
cycler.", 
"  Human vimentin amplifcation curve data (measured during elongatio phase 
at 68.5 deg Celsius) for 96 replicate samples in a Bio-Rad iQ5 thermo 
cycler.", 
"  Human vimentin melting curve data for 96 replicate samples in a Bio-Rad 
iQ5 thermo cycler.", 
...
quantities. ", 
"  Quantitive PCR with a hydrolysis probe and DNA binding dye (EvaGreen) 
for MLC-2v measured at 59.5 degree Celsius (annealing temperature), 68.5 
...
derived from a simulation such as the AmpSim function.", 
"  A Helicase Dependent Amplification (HDA) of HRPT1 (Homo sapiens 
hypoxanthine phosphoribosyltransferase 1), performed at different input 
DNA quantities using the Bio-Rad iQ5 thermo cycler.", 
"  Helicase Dependent Amplification in the VideoScan HCU of HRPT1 (Homo 
...
Platform. The HDA was performed at 65 degree 
Celsius. Two concentrations of input DNA were used.", 
"  Helicase Dependent Amplification (HDA) of Vimentin (Vim) in the 
VideoScan Platform. The HDA was performed at 65 
degree Celsius with three dilutions of input DNA."
), dat.types = c("capillary convective PCR (ccPCR)", "standard qPCR - 
commercial thermo cyclers", 
"standard qPCR - commercial thermo cyclers", "standard qPCR - commercial 
thermo cyclers", 
"standard qPCR - commercial thermo cyclers", "standard qPCR - commercial 
thermo cyclers", 
"standard qPCR - commercial thermo cyclers", "standard qPCR - commercial 
...
"standard qPCR - commercial thermo cyclers", "standard qPCR - commercial 
thermo cyclers", 
"standard qPCR - experimental thermo cyclers", "standard qPCR - 
experimental thermo cyclers", 
"simulations", "simulations", "simulations", "simulations", "Isothermal 
Amplifcation - Helicase Dependent Amplification", 
"Isothermal Amplifcation - Helicase Dependent Amplification", 
"Isothermal Amplifcation - Helicase Dependent Amplification", 
"Isothermal Amplifcation - Helicase Dependent Amplification")), .Names = 
c("dat.names", 
