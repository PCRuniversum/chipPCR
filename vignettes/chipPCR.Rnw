...
All datasets used in following examples can be loaded in the same manner. They are also 
automatically available after loading whole package.
...
was used to smooth the data. The following code is an representative example for 
