...
D3) were used. The amplifcation curves were smoothed by a moving average 
(windowsize 3) and baselined by a a robust linear regression by computing 
MM-type regression estimator. The th.cyc function was used to determine 
...
amplifcation curve was simulated with the AmpSim function."
...
fig18_cap <- "\\textsl{bg}.\\textsl{max} tries to estimate the range 
between the background and the plateau phase of an amplification reaction. 
\\emph{(A)} in absence and \\emph{(B)} presence of noise."

fig18_scap <- "Estimate the range between the background and the plateau 
phase of an amplification reaction."
...
a trend. The function fixNA was empirically tested and relies on a linear 
...
of an amplification reaction. The parameter spline on fixNA enables a 
...
Savitzky_1964}. The function fixNA imputes missing values in a single 
...
\textsl{lm.coefs} is a wrapper around functions performing normal (least 
squares) and robust linear regression. If the robust linear regression is 
not possible, \textsl{lm.coefs} will perform linear regression using the 
...
# Simulate an amplifcation curve with 40 cycles using the the AmpSim 
# function.
...
amplification is used. The parameter $logy$ is $FALSE$ by defalt. To the 
...
precise peak location while the approaximate $SDM$ is calculated from the 
...
common problem. An alternative technique to determine the $FDM$ of $SDM$ is 
...
<<effcalc_VIM_MLC,fig.show='hold',fig.cap=fig19_cap,fig.scap=fig19_scap>>=
...
if the $noise$ parameter is set $TRUE$. This is due the use of the 
\textsl{rnorm} (\emph{stats}) function to simulate noise. If data need to 
be replicated identically use \textsl{set.seed(123)} to alter the random 
number generator (RNG) state. For example, the amplification curves of 
Figure~\ref{fig:AmpSim_effcalc}~\emph{A} are generated with the same 
starting parameter of \textsl{AmpSim} but noise was added. 
\textsl{AmpSim.gui} is a \emph{shiny} GUI (graphical user interface) 
implementation for \textsl{AmpSim}. Further details are described in 
section ``Graphical User Interface \& data import and export''. 
\textsl{AmpSim} was also used to illustrate the the \textsl{inder} 
function (Figure~\ref{figure:SDM}), the \textsl{fixNA} function 
...
<<bgmax_ccPCR,fig.cap=fig20_cap,fig.scap=fig20_scap,fig.show='hold',message=FALSE,results='hide'>>=
...
colors <- rainbow(8)
plot(NA, NA, xlim = c(0,75), ylim = c(-120,1300), xlab = "Time [min]", 
     ylab = "Voltage (micro V)", main = "ccPCR")

for (i in c(1,3,5,7)) {
    x <- capillaryPCR[1L:750, i]
    y <- CPP(capillaryPCR[1L:750, i], capillaryPCR[1:750, i + 1], 
	     method = "mova", trans = TRUE, bg.range = c(1,105), bg.outliers = TRUE)[["y.norm"]]
    res.bg <- summary(bg.max(x, y))
    lines(x, y, type = "b", pch = 20, col = colors[i])
    arrows(res.bg["bg.stop"], -140 + i * 10, res.bg["amp.stop"], -140 + i * 10, 
	   col = colors[i], code = 3, lwd = 2, length = 0.1, angle = 90)
    text(10, 1200 - i * 50, 
	 paste("bg.start: ", res.bg[1], ", bg.stop: ", res.bg[2], 
	       ", amp.stop: ", res.bg[4]), col = colors[i])
  }
@
