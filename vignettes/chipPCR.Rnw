...
fig13_scap <- 
"Helicase Dependent Amplification (HDA) of Vimentin (Vim)."
...
curve shape of a qIA and ccPCR do not necessarily follow a S-shaped structure 
and the measurement is time-based (continuous, not mandatory equidistant) in 
contrast to a cycle-based (discontinuous) measurement of qPCRs. The number of 
measure points in most cases higher than qPCR.
...
are also automatically available after loading whole package. Recently we added 
the $\emph{chipPCR}$ package \cite{blagodatskikh_2014} to CRAN. The 
$\emph{chipPCR}$ can be used to read the Real-time PCR Data Markup Language 
...
model (Equation~S\ref{eq:richards}). The function is commonly used for the 
simulation of amplification curves \cite{spiess_2008, ritz_2008, gerhard_2014}.
...
The code (``server.R'', ``ui.R'') of the \emph{shiny} app is accessible too. All 
parameters (e.g., Cq value, Baseline) of the \textsl{AmpSim} function are 
accessible. \emph{(Bottom)} In addition shows \textsl{AmpSim}.\textsl{gui} the 
plot output and the textual results of \textsl{bg}.\textsl{max}}
...
processes its is reasonable to estimate the missing values from a trend. The 
function \textsl{fixNA} was empirically tested and relies on a linear or 
non-linear trend estimation. Other smoothing functions such as the 
...
Data within an experiment have in most cases different minimum and maximum 
values. For the visualization of the data it is ofter better to scale the data 
within a defined range. Eventually, this helps to grasp the data faster. In 
particular, a comparison of data from different measurements and/or scaling is 
easier if data are normalized. \textsl{normalizer} is a function to normalize 
any data set. It is possible to chose from different methods (see Details). This 
is recommended if the data from an experiment have considerable variation 
regarding the background and plateau signal.
