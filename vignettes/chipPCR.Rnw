...
<<fixNA_data,fig.show='hold',fig.cap=fig20_cap,fig.scap=fig20_scap>>=
library(qpcR)
library(chipPCR)
cols <- adjustcolor(2:4, 0.7)
...
\centering
\scalebox{0.6}{
\includegraphics{fixNA_test.png}
}
...
introduced into the ``reps384'' data set from the $\emph{qpcR}$ package. We found no significant difference } 
