...
influence of imputation on real-world data. This data set consists of 380 
replicate amplification curves (see documentation of the $\emph{qpcR}$ package 
for details). Our in-silico experiment was designed to introduce either or three 
missing values. The missing values were artificially introduced into the data at 
random cycle numbers. We separated the amplification curve in three fixed 
regions (``Linear phase'', ``Exponential phase'', ``Plateau phase'') and 
...
root-mean-squared-error (NRMSE). The smaller the NRMSE the better the imputation 
is. Using this approach we compared the imputation by ``linear approximation'' 
and ``spline approximation''. Our results show that the imputation with the 
...
rect(0.8, min(reps384[, -1]), 10.2, max(reps384[, -1]), border = NA, col = "red")
rect(10.8, min(reps384[, -1]), 33.2, max(reps384[, -1]), border = NA, col = "blue")
rect(33.8, min(reps384[, -1]), 45, max(reps384[, -1]), border = NA, col = "green")
