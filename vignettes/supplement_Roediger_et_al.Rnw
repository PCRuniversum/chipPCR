...
<<captures,warning=FALSE,message=FALSE,echo=FALSE>>=
...
text) and analyzed with \\textsl{effcalc}. The amplification efficiency approximately at 
87.3~\\%."
...
\subsection{Isothermal amplification -  Helicase Dependent Amplification of Vimentin}
...
mtext("A", cex = 2, side = 3, adj = 0)
  lines(C85[, 2], C85[, 3], type = "b", col = 1, pch = 20)
  lines(C85[, 4], C85[, 5], type = "b", col = 2, pch = 20)
  lines(C85[, 6], C85[, 7], type = "b", col = 3, pch = 20)
...
# lrg <- "least"
# for (i in c(2,4,6)) {
#   y.tmp <- CPP(C85[xr, i], C85[xr, i + 1], method = sm, bg.range = br, trans = TRUE)$y.norm
#   Ct.tmp <- th.cyc(C85[xr, i], y.tmp, r = 0.05, linear = FALSE)
#   #abline(v = Ct.tmp[1])
...
and \textsl{diffQ2} used for a precise peak location while the approaximate 
...
D1 <- cbind(C54[1:35, 1], CPP(C54[1:35, 1], C54[1:35, 2], trans = TRUE, bg.range = c(1,8))[["y.norm"]])
